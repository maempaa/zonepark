...
- **Cómo llega.** Cada ticket nace con un `token_publico` de 128 bits. Al
  ...
  cliente lo perdió— el operario elige WhatsApp o correo. Por WhatsApp
  teclea el número y el botón abre la conversación con el enlace ya
  escrito; por correo teclea la dirección, elige con qué redactor escribe
  —Gmail, Outlook, Yahoo o la aplicación del dispositivo— y se abre el
  borrador listo. Los dos datos quedan guardados contra la placa, y
  guardar uno no borra el otro: la próxima visita de ese vehículo ya
  vienen puestos, y se pueden borrar desde el mismo sitio. El consecutivo (`S1-000002`) y la placa no sirven
