"""A qué número mandarle el recibo de cada placa.
...
from app.services.contactos import TelefonoInvalido, contacto_de, olvidar, recordar
...
async def ver_contacto(
    ...
    if contacto is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Esa placa no tiene número guardado")
    return contacto
...
async def recordar_contacto(
    ...
    try:
        return await recordar(
            session, tenant=tenant, placa=placa, telefono=datos.telefono
        )
...
async def olvidar_contacto(
    ...
    """Borra el número. Es el derecho del cliente a que no lo guarden."""
